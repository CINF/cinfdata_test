...
$query = "SELECT id, unix_timestamp(time), comment FROM " .  $settings["measurements_table"] . " where type = " . $settings["type"] . " order by time desc limit 100";
...
echo("<pre>");
echo(shell_exec("python /var/www/volvo/gas_analysis.py " . $param["id"]));
echo("</pre>");
...
print("<select name=\"idlist[]\" multiple size=\"7\">\n");
for($i=0;$i<count($idlist);$i++){
    $selected = (in_array($idlist[$i],$plotted_ids)) ? "selected" : "";
    echo("<option value=\"" . $idlist[$i] . "\" " . $selected . ">" . $idlist[$i] . ": " . $commentlist[$i] . "</option>\n");
}
print("</select>\n");
print("</p>\n\n");
