...
if ($id_array == ""){ // If no id is given, get metadata for the lastest measurement
	$query = "SELECT id, unix_timestamp(time), timestep FROM measurements where type = 4 order by time desc limit 1";
	...
	foreach($id_array as $id){
		$query = "SELECT UNIX_TIMESTAMP(time), timestep FROM measurements where id = " . $id;
		$result  = mysql_query($query,$db);  
		$row = mysql_fetch_array($result);
		$time[] = $row[0];
		$timestep[] = $row[1];
...
// Fill up the array of stepsizes
foreach($id_array as $id){
	$query = "SELECT x FROM xy_values where measurement = " . $id . " order by id limit 2";
	$stepsize[] = get_xy_stepsize($query,$db);
}


// Get a list of id's that satisfies a search criterion
// These will be shown in a selection list
$query = "SELECT id FROM measurements where type = 4 order by time desc limit 100";
$result  = mysql_query($query,$db);  
...
<img style="float:left" src="massspectrum.php?scaletype=<?=$scaletype?>&manualscale=<?=$manualscale?>&ymax=<?=$ymax?>&ymin=<?=$ymin?><?=$html_formatted_idlist?>">
...
foreach($id_array as $id){
	...
	echo("<b>Step size:</b> " . $stepsize[$i] . "<br>\n");
	echo("<b>Time pr. step:</b> " . $timestep[$i] . "s<br>\n");
	echo("<b>Time pr. amu:</b> " . $timestep[$i]/$stepsize[$i] . "s</p>\n");
	echo("</div>\n");
	$i++;
}
?>
...
<form action="read_massspectrum.php" method="get">
<p><b>Id:</b>
<select name="idlist[]" multiple size="4">
<?
