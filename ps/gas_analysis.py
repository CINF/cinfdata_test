...
# Command to connect to mySQL database and get
# "dataset", which is the data to be analyse.
#cnxn = pyodbc.connect('DSN=Robert')
cursor = cnxn.cursor()
cursor.execute("select x,y FROM xy_values_%s where measurement = %i order by id;"  % (setup, dataset))
data = np.array(cursor.fetchall())

# Getting time of the mass-scan mySQL database
cursor.execute("""SELECT unix_timestamp(time) FROM measurements_%s where
               id=%i;""" % (setup, dataset))
